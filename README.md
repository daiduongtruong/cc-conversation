...
This installs globally for all projects. On first Claude response in any project, it auto-creates `.conversations/`.
...
| Hook | When | What |
|------|------|------|
| **Stop** | After each Claude response | Syncs conversation to `.conversations/sessions/<id>.md` |
...
5. Extracts only human/assistant text (no tool use, no tool results)
6. Writes grep-friendly markdown with timestamped `## User` / `## Assistant` headers
7. Commits to the `.conversations/` git repo
...
└── .conversations/          # separate git repo
    ...
    ├── sessions/
    │   └── <session-id>.md  # one file per session
